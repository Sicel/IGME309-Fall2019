...
void MyCamera::Rotate(float horizontal, float vertical)
{
	m_v3Forward = m_v3Target - m_v3Position; // sets camera's forward vector

	RotateYaw(horizontal);
	RotatePitch(vertical);
...
void MyCamera::RotatePitch(float vertical)
{
	vector3 up = glm::cross(m_v3Forward, m_v3Right); // gets up vector

	vector3 targetViewVert = (glm::normalize(m_v3Forward) * cos(vertical)) + (glm::normalize(up) * sin(vertical)); // gets target vector

	targetViewVert = glm::normalize(targetViewVert); // normalizes target vector

	m_v3Target = targetViewVert + m_v3Position; // sets target
...
void MyCamera::RotateYaw(float horizontal)
{
	...
	targetViewHorz = glm::normalize(targetViewHorz); // normalizes target vector

	m_v3Target = targetViewHorz + m_v3Position; // sets target
}